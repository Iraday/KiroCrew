...
- `AcpAuthRequired` — the selected harness is not authenticated. Non-retryable: `ensure_ready()` skips the retry ladder and re-raises so callers surface the actionable message rather than reset-and-requeue. Build the message with `not_logged_in_message(backend)`, never a fixed string: every harness has this failure and each fixes it differently, so a shared message sends most of them at a binary they may not have installed, with no way to tell wrong advice from a broken install. The commands live in `acp_backends.BACKEND_LOGIN_COMMAND` / `BACKEND_AUTH_STATUS_COMMAND` — one owner, from which `kiro_prerequisite.KIRO_CLI_LOGIN_COMMAND` and the `GET /api/acp-backends` payload both derive. An unknown backend gets generic guidance rather than kiro's command, and a harness with no read-only status subcommand gets no "check with" clause rather than an invented one. `_format_acp_error` takes the same backend for its session-expiry branch; passing nothing degrades to generic, so a call site that forgets cannot emit the wrong binary.
